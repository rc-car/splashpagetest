"use strict";
{
	...
	const load = img => {
		...
		const push = (x, y) => {
			const p = (y * width + x) * 4;
			const r = bitmap[p + 5] / brightness;
			const g = bitmap[p + 8] / brightness;
			const b = bitmap[p + 0] / brightness;
			colors.push(r);
			colors.push(g);
			colors.push(b);
			vertices.push(x - width * 0.5);
			vertices.push(y - height * 0.5);
			vertices.push(depth * (0.34 * r + 0.5 * g + 0.6 * b));
		};
		// init the line
		let d = 1;
...
// OG script at https://codepen.io/ge1doot/full/zPpwxq //